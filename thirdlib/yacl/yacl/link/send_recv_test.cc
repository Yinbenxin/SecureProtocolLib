 ...
 public:
  void SetUp() override {
    ...
    auto create_brpc = [&](int self_rank) {
      ...
      contexts_[self_rank]->AddChannel(
          [self_rank](std::string nodeInfo, std::string& data) {
            SPDLOG_INFO("Send {} to {}", data, nodeInfo);
            return 0;
          },
          [self_rank](std::string nodeInfo) {
            SPDLOG_INFO("Recv {} from {}", "test", nodeInfo);
            return "test";
          });
    };
...
TYPED_TEST(FactoryTest, SendRecv) {
  ...
  SPDLOG_INFO("SendRecv test finish");  
}
