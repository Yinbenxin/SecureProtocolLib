...
// control message keys (declare here to use in header inline methods)
...
class IChannel {
 public:
  ...
  virtual void SetChannel(std::function<int(const std::string &, std::string &)> sendCb, std::function<std::string(const std::string&)> recvCb) = 0;
};
...
class Channel : public IChannel, public std::enable_shared_from_this<Channel> {
 ...
 protected:
  ...
  void SendImpl(const std::string& key, ByteContainerView value,
    ...
    SPDLOG_INFO("{} send {}, value_size={}", link_->LocalRank(), key, value.size());
    // do not intercept control messages: ack/fin/connect_*
    if (sendCallback_ && key != kAckKey && key != kFinKey && key.rfind("connect_", 0) != 0) {
      std::string str((char*)value.data(), value.size());
      sendCallback_(key, str);
      return;
    }
