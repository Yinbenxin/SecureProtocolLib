...
#include "psi/algorithm/circuit/he_unit.h"
#include <stdexcept>
#include <vector>
#include <algorithm>
...
Ciphertext PaillierHE::Encrypt(const BigInt& plaintext) {
  if (!encryptor_) {
...
BigInt PaillierHE::Decrypt(const Ciphertext& ciphertext) {
...
size_t PaillierHE::Pack_Encrypt(const std::vector<std::vector<int64_t>>& data, std::vector<std::string>&ciphertexts){
    ...
    std::vector<std::vector<yacl::math::BigInt>> packed_data = PackDataToBigInt(data, data_size_each_ciphertext);
...
Ciphertext PaillierHE::Add(const Ciphertext& ciphertext, const BigInt& plaintext) {
  if (!evaluator_) {
    throw std::runtime_error("Evaluator not initialized");
  }
...
Ciphertext PaillierHE::Mul(const Ciphertext& ciphertext, const BigInt& plaintext) {
  if (!evaluator_) {
    throw std::runtime_error("Evaluator not initialized");
  }
...
Ciphertext PaillierHE::Sub(const Ciphertext& ciphertext, const BigInt& plaintext) {
  if (!evaluator_) {
    throw std::runtime_error("Evaluator not initialized");
  }
...
BigInt pack_int(const std::vector<int64_t>& packed, int64_t num_in_one_pack, int64_t block_size) {
    // 检查输入参数
...
std::vector<int64_t> unpack_int(const BigInt& pack_data, int64_t num_in_one_pack, int64_t block_size) {
    ...
    BigInt base(1);
    base = base << block_size;
    BigInt mask = base - BigInt(1);
    BigInt sign_threshold = base >> 1; // 2^(block_size-1)

    BigInt reminder_data = pack_data;
    // 从最低位开始解包
    for (int64_t i = 0; i < num_in_one_pack; ++i) {
        BigInt low = reminder_data & mask;  // 取低 block_size 位
        int64_t value;
...
// 数据打包函数：将原始数据打包成BigInt向量
std::vector<std::vector<yacl::math::BigInt>> PackDataToBigInt(
    ...
    std::vector<std::vector<yacl::math::BigInt>> packed_data(data.size());
    ...
    for (size_t i = 0; i < data.size(); i++) {
        for (size_t j = 0; j < ciphertext_size; j++) {
            ...
            // 打包并添加到结果中
            yacl::math::BigInt packed = heu::lib::algorithms::paillier_z::pack_int(data_to_pack, data_size_each_ciphertext);
