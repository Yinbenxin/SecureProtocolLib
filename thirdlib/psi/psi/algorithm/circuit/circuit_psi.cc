...
// 数据解包函数：将BigInt向量解包回原始数据
std::vector<std::vector<int64_t>> UnpackDataFromBigInt(
    const std::vector<std::vector<yacl::math::BigInt>>& packed_data,
...
std::vector<std::vector<int64_t>> ciphertext_random(const std::shared_ptr<paillier::PaillierHE>& HE, std::vector<std::string>& ciphertext_str) {
    ...
    for (size_t i = 0; i < ciphertext_str.size(); i++)
    {
        ...
        for (size_t j = 0; j < data_vec.size(); j++) {
          auto random_data_uint =
              gen_random_data(1 - (1ULL << 60), (1ULL << 61) - 1, 16);
          ...
          yacl::math::BigInt data_item(data_vec[j], 10);
          paillier::Ciphertext ciphertext_MTint(data_item);
          auto ciphertext_random_MTint =
              HE->Sub(ciphertext_MTint, random_data_MTint);
          ciphertexts[i] =
              ciphertexts[i] + "|" + ciphertext_random_MTint.ToString();
        }
...
std::vector<std::vector<int64_t>> DecryptAndGetShare(
    ...
    std::vector<std::vector<yacl::math::BigInt>> intersect_data_MTint(intersect_enc_data_mask_self.size());
    ...
    std::vector<std::vector<int64_t>>intersect_data_self = UnpackDataFromBigInt(intersect_data_MTint, self_raw_size, 16);
