...
void shuffle_items(std::vector<std::string>& peer_items,
                   std::vector<std::string>& peer_enc_data);
std::vector<std::vector<int64_t>> ciphertext_random(
    const std::shared_ptr<paillier::PaillierHE>& HE,
    std::vector<std::string>& ciphertext_str);
std::vector<int64_t> gen_random_data(size_t min, size_t max, size_t len);
std::shared_ptr<paillier::PaillierHE> InitializeHE(
    const std::shared_ptr<yacl::link::Context>& link_ctx);
}  // namespace psi::circuit
