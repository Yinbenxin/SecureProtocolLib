...
class PaillierHE {
 public:
   ...
   * @return 密文
   */
  Ciphertext Encrypt(const BigInt& plaintext);
   ...
   * @return 明文
   */
  BigInt Decrypt(const Ciphertext& ciphertext);
   ...
   * @return 相加后的密文
   */
  Ciphertext Add(const Ciphertext& ciphertext, const BigInt& plaintext);
   ...
   * @return 相乘后的密文
   */
  Ciphertext Mul(const Ciphertext& ciphertext, const BigInt& plaintext);
   ...
   * @return 相减后的密文
   */
  Ciphertext Sub(const Ciphertext& ciphertext, const BigInt& plaintext);
...
/**
 * 将多个int64_t值打包成一个BigInt
 * @param packed 要打包的int64_t值向量
 * @return 打包后的BigInt值
 */
BigInt pack_int(const std::vector<int64_t>& packed, int64_t num_in_one_pack=16, int64_t block_size = 64);

/**
 * 将打包的BigInt解包成多个int64_t值
 * @param pack_data 打包的BigInt值
 * @return 解包后的int64_t值向量
 */
std::vector<int64_t> unpack_int(const BigInt& pack_data, int64_t num_in_one_pack=16, int64_t block_size = 64);
